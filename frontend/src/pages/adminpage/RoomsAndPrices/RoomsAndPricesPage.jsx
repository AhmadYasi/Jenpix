...
// ── Data ──────────────────────────────────────────────────────────────────────
const INITIAL_ROOMS = [
  { id:1, name:"Gutedel",   roomNo:"Room 1", type:"Suite",  occupancy:2, occupancyNote:"",               extraBed:false, priceBase:134, breakfastPP:26, extraBedPrice:null, features:["wifi","tv","desk"],        description:"", status:"active", img:"https://images.unsplash.com/photo-1631049307264-da0ec9d70304?w=200&q=70" },
  { id:2, name:"Bacchus",   roomNo:"Room 3", type:"Double", occupancy:2, occupancyNote:"",               extraBed:false, priceBase:109, breakfastPP:26, extraBedPrice:null, features:["wifi","tv","desk","bath"],  description:"", status:"active", img:"https://images.unsplash.com/photo-1566665797739-1674de7a421a?w=200&q=70" },
  { id:3, name:"Riesling",  roomNo:"Room 4", type:"Suite",  occupancy:2, occupancyNote:"",               extraBed:true,  priceBase:134, breakfastPP:26, extraBedPrice:30,   features:["wifi","tv","desk"],        description:"", status:"active", img:"https://images.unsplash.com/photo-1618773928121-c32242e63f39?w=200&q=70" },
  { id:4, name:"Scheurebe", roomNo:"Room 5", type:"Double", occupancy:2, occupancyNote:"",               extraBed:false, priceBase:109, breakfastPP:26, extraBedPrice:null, features:["wifi","tv","desk","bath"],  description:"", status:"active", img:"https://images.unsplash.com/photo-1595576508898-0ad5c879a061?w=200&q=70" },
  { id:5, name:"Burgunder", roomNo:"Room 6", type:"Suite",  occupancy:3, occupancyNote:"2 Adult, 1 Kid", extraBed:true,  priceBase:134, breakfastPP:26, extraBedPrice:30,   features:["wifi","tv","desk"],        description:"", status:"active", img:"https://images.unsplash.com/photo-1582719478250-c89cae4dc85b?w=200&q=70" },
  { id:6, name:"Rivaner",   roomNo:"Room 7", type:"Single", occupancy:1, occupancyNote:"",               extraBed:false, priceBase:76,  breakfastPP:13, extraBedPrice:null, features:["wifi","tv"],               description:"", status:"active", img:"https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=200&q=70" },
  { id:7, name:"Regent",    roomNo:"Room 8", type:"Suite",  occupancy:2, occupancyNote:"",               extraBed:true,  priceBase:134, breakfastPP:26, extraBedPrice:30,   features:["wifi","tv","desk","bath"],  description:"", status:"active", img:"https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=200&q=70" },
];
...
// ── Room Modal ────────────────────────────────────────────────────────────────
const EMPTY_FORM = {
  name:"", roomNo:"", type:"Suite", occupancy:2, occupancyNote:"",
  extraBed:false, priceBase:"", breakfastPP:"", extraBedPrice:"",
...
function RoomModal({ room, onClose, onSave }) {
  ...
  const [images, setImages]   = useState(isEdit && room.img ? [room.img] : []);
      ...
      <div className="rm-modal rm-modal--wide">
        ...
        <div className="rm-header">
          <div className="rm-header__left">
            ...
            <div>
              <h2 className="rm-header__title">{isEdit ? "Edit Room" : "Add New Room"}</h2>
              <p className="rm-header__sub">{isEdit ? `Editing: ${room.name}` : "Fill in the room details and upload images"}</p>
            </div>
          </div>
        ...
        <div className="rm-body rm-body--two-col">
          ...
          <div className="rm-col-left">
            ...
            <div className="rm-section">
              ...
              <div className="rm-row rm-row--2">
                ...
                <div className="rm-field">
                  ...
                  <input className={`rm-input ${errors.roomNo ? "rm-input--err" : ""}`} placeholder="e.g. 1" value={form.roomNo} onChange={set("roomNo")} />
                  {errors.roomNo && <p className="rm-err">{errors.roomNo}</p>}
                </div>
              </div>
              <div className="rm-row rm-row--2">
                <div className="rm-field">
                  <label className="rm-label">Room Type <span className="rm-req">*</span></label>
                  <select className="rm-input rm-select" value={form.type} onChange={set("type")}>
            ...
            <div className="rm-section">
              ...
              <div className="rm-row rm-row--3">
                ...
                <div className="rm-field">
                  <label className="rm-label">Breakfast Price (per person) <span className="rm-req">*</span></label>
                  <div className="rm-input-prefix">
                    <span className="rm-prefix">€</span>
          ...
          <div className="rm-col-right">
            <div className="rm-section rm-section--sticky">
              ...
              <label className="rm-label">Upload Images <span className="rm-req">*</span></label>
              <p className="rm-hint" style={{marginBottom:10}}>You can upload up to {MAX_IMAGES} images.</p>
              ...
              {images.length > 0 && (
                <div className="rm-img-section">
                  ...
                  <p className="rm-hint">{images.length} / {MAX_IMAGES} images uploaded. First image is the main photo.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="rm-footer">
          <button className="rm-btn rm-btn--ghost" onClick={onClose}>Cancel</button>
...
export default function RoomsAndPricesPage() {
  ...
  const extraBedPrice = 30;
  const breakfastPrice= 26;

  const handleSave = (form) => {
    if (modal === "add") {
      setRooms(prev => [...prev, { ...form, id: Date.now() }]);
    } else {
      setRooms(prev => prev.map(r => r.id === modal.id ? { ...r, ...form } : r));
    }
  };

  const handleDelete = (id) => { setRooms(prev => prev.filter(r => r.id !== id)); setMenuOpen(null); };
  ...
  return (
    <div className="rp-layout">
        ...
        <div className="rp-content">
          <div className="rp-top-row">
            <div className="rp-stats">
              ...
              <div className="rp-stat"><p className="rp-stat__label">Extra Bed Price</p><p className="rp-stat__value">€{extraBedPrice}</p><p className="rp-stat__sub">Per night</p></div>
              <div className="rp-stat"><p className="rp-stat__label">Breakfast Price</p><p className="rp-stat__value">€{breakfastPrice}</p><p className="rp-stat__sub">Per person</p></div>
            </div>
            <div className="rp-actions">
              <button className="rp-btn rp-btn--primary" onClick={() => setModal("add")}>
            ...
            <table className="rp-table">
              <thead>
                ...
                <tr className="rp-subhead">
                  ...
                  <th className="rp-subth">Breakfast (pp)</th>
                    ...
                    <td className="rp-td">
                      <div className="rp-action-cell">
                        ...
                        <div className="rp-menu-wrap">
                          <button className="rp-action-more" onClick={() => setMenuOpen(menuOpen===room.id?null:room.id)}>⋮</button>
                          {menuOpen === room.id && (
                            <div className="rp-dropdown">
                              <button className="rp-dropdown-item" onClick={() => { setModal(room); setMenuOpen(null); }}>
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                Edit Room
                              </button>
                              <button className="rp-dropdown-item rp-dropdown-item--danger" onClick={() => handleDelete(room.id)}>
      ...
      {menuOpen && <div className="rp-menu-overlay" onClick={() => setMenuOpen(null)} />}
    </div>
  );
}